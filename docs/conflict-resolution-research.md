...
风险提示使用 32 个派生 seed 做快速胜率采样，但玩家可见文案不显示“数据模拟”或裸露系统术语。文案解析优先级为：世界包 `conflictRules` 提示词 → 本地默认文案 `src/content/default-conflict-copy.ts`。因此所有剧本都至少有可用提示，但剧本包可以完全替换成自己的世界观风格：
